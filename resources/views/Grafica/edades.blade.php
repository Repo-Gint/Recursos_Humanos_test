...
<script>
$('#grafico').load('{{ route('graficos.edades_1') }}');
    $(document).ready( function() { 
        $("#Actualizar").click( function() {   
        if(validar()){  
            var url = '{{ route('graficos.edades_1', ['inicio', 'fin']) }}';
...
function exportCharts() {
  ...
  function generatePDF() {
    ...
    // Add bigger chart
    layout.content.push({
      "image": charts["chartEdades"].exportedImage,
      "width": 810,
      "height":310
    });
    ...
    chart["export"].toPDF(layout, function(data) {
      this.download(data, "application/pdf", "gim_edades.pdf");
    });
