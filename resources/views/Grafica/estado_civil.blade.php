...
<script>
$('#grafico').load('{{ route('graficos.estado_civil_1') }}');
    $(document).ready( function() { 
        $("#Actualizar").click( function() {   
        if(validar()){  
            var url = '{{ route('graficos.estado_civil_1', ['inicio', 'fin']) }}';
...
function exportCharts() {
  ...
  function generatePDF() {
    ...
    // Add bigger chart
    layout.content.push({
      "image": charts["estado_civil"].exportedImage,
      "width": 850,
      "height":310
    });
    ...
    chart["export"].toPDF(layout, function(data) {
      this.download(data, "application/pdf", "gim_estado_civil.pdf");
    });
