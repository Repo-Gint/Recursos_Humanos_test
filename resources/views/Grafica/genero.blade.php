...
<script>
$('#grafico').load('{{ route('graficos.genero_1') }}');
    $(document).ready( function() { 
        $("#Actualizar").click( function() {   
        if(validar()){  
            var url = '{{ route('graficos.genero_1', ['inicio', 'fin']) }}';
...
function exportCharts() {
  ...
  function generatePDF() {
    ...
    // Add bigger chart
    layout.content.push({
      "image": charts["chartdivPie"].exportedImage,
      "width": 850,
      "height":310
    });
    ...
    chart["export"].toPDF(layout, function(data) {
      this.download(data, "application/pdf", "gim_genero.pdf");
    });
